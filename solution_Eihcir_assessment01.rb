...
class Array
  ...
  def my_unzip
    transpose.each do |subzip|
      subzip.delete_if{|el| el.nil?}
    end
  end
...
class Hash
  ...
  def my_hash_inject(accumulator = nil, &prc) #<<=delete this for fun
    accumulator_not_given = accumulator.nil?
    accumulator ||= self.values.first

    self.values.each_with_index do |value, idx|
      next if accumulator_not_given && idx == 0
...
def fibonaccis(n)
	...
	([fibonacci(n-1) + fibonacci(n-2)] + fibonaccis(n-1)).sort
end
...
def richienacci(n)
	return 3 if n == 1
	fibonaccis(n+1).inject(&:+) + fibonaccis(n).inject(&:+)
end
...
def richienaccis(n)
	return [3] if n == 1
	return [3,6] if n == 2
	([richienacci(n)] + richienaccis(n-1)).sort
end
